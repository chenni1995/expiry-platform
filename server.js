...
app.get('/api/export-query', (req, res) => {
  try {
    const { sampleNos } = req.query;
    if (!sampleNos) {
      return res.status(400).json({ error: '请提供样本编号' });
    }
    
    const sampleNoArray = sampleNos.split(',').map(s => s.trim()).filter(s => s);
    if (sampleNoArray.length === 0) {
      return res.status(400).json({ error: '无有效样本编号' });
    }
    
    const placeholders = sampleNoArray.map(() => '?').join(',');
    const stmt = db.prepare(`SELECT sample_no, product_name, expiry_date, generate_date FROM products WHERE sample_no IN (${placeholders})`);
    const rows = stmt.all(...sampleNoArray);

    const data = sampleNoArray.map(no => {
      const found = rows.find(r => r.sample_no === no);
      return found || { sample_no: no, product_name: '', expiry_date: '未找到', generate_date: '' };
    });
